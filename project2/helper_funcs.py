...
def unpack_options(options):
	...
	options >>= 1
	return answer_dict
def pack_options(rcode, RA, RD, TC, AA, OPCODE, QR):
	answer = 0x0000
	#~ items = struct.pack()
	answer |= rcode & 0xf
	answer |= (RA  & 0x1) << 7
...
class DNSQuery:
	...
	def dict_to_header(dictionary):
		return DNSQuery.make_header(dictionary['ID'], dictionary['QDCOUNT'], dictionary['ANCOUNT'],dictionary['NSCOUNT'],dictionary['ARCOUNT'],
			dictionary['RCODE'],dictionary['RA'],dictionary['RD'],dictionary['TC'],dictionary['AA'],dictionary['OPCODE'],dictionary['QR'])
	@staticmethod
	def make_header(qid=0, q_number = 0, a_number = 0, auth_num = 0, add_r = 0, rcode = 0, RA = 0, RD = 0, TC = 0, AA=0, OPCODE=0, QR=0):
		dns_header_string = '!HHHHHH'
		dns_options = pack_options(rcode, RA, RD, TC, AA, OPCODE, QR)
	...
	def make_RR(query_so_far, record_dict):
		...
		if record_dict['TYPE'] in NAME_RR_SET:
			temp_data = DNSQuery.name_to_bytes(query_so_far + record + rdata, len(query_so_far + record + rdata), data_to_parse[0])
			rdata += temp_data
			data_to_parse = data_to_parse[1:]
		elif record_dict['TYPE'] in TWO_NAME_RR_SET:
			temp_data = bytearray()
			for name in data_to_parse[:2]:
				temp_data = DNSQuery.name_to_bytes(query_so_far + record + rdata, len(query_so_far + record + rdata), name)
				rdata += temp_data
